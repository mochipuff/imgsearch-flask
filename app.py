...
class RequestManager:
    def __init__(self):
        self.headers = {
            ...
            "Cookie": "SRCHHPGUSR=ADLT=OFF&NRSLT=50; SRCHD=AF=NOFORM; MUID=0123456789ABCDEF0123456789ABCDEF",
...
class RobustImageScraper:
        ...
        for tag, attrs in selectors:
            ...
            for element in elements:
                try:
                    ...
                    if img_data and img_data.original_url and img_data.original_url.startswith("http"):
                        if not any(i.original_url == img_data.original_url for i in images):
    ...
    def _extract_image_data(self, element) -> Optional[ImageResult]:
        m_attr = element.get("m") or element.get("data-m")
        if m_attr:
            try:
        ...
        img_tag = element.find("img") or (element if element.name == "img" else None)
        if img_tag:
    ...
    def search(self, query: str, limit: int = 10, offset: int = 1) -> Dict:
        ...
        url = f"https://www.bing.com/images/async?q={encoded_query}&first={offset}&count={limit}&adlt=off&mmasync=1"
